...
const Navbar = () => {
    ...
    return (
        <nav className="flex justify-between items-center py-5 container mx-auto">
            <Link className="text-3xl font-bold" href={"/"}>Md<span className="text-5xl font-bold text-orange-600">.</span> Tanim</Link>
            {/* desktop view */}
            <div className="hidden md:flex">
                {links.map((link) => (
                    <Link className={`mr-5 ${pathName == link.path && 'text-orange-600 border-b-2 pb-1'}`} href={link.path} key={link.path}>
                        {link.name}
                    </Link>
                ))}
            </div>
